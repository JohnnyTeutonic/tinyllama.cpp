...
class ModelBase:
    ...
    def prepare_tensors(self):
        ...
        for name, data_torch_from_getter in chain(self.generate_extra_tensors(), self.get_tensors()):
            # we don't need these
            if name.endswith((".attention.masked_bias", ".attention.bias", ".rotary_emb.inv_freq")):
                continue

            # Capture the original dtype for logging and to ensure modify_tensors gets the original tensor.
            original_dtype = data_torch_from_getter.dtype
            tensor_for_modification = data_torch_from_getter # This tensor retains the original dtype
            ...
            # self.modify_tensors will now operate on tensor_for_modification (e.g., BF16)
            for new_name, modified_tensor_output in (self.modify_tensors(tensor_for_modification, name, bid)):
                
                # Now, convert to FP32 if the modified tensor isn't already F16 or F32.
                # This is primarily for the case where --outtype f32 is specified and original was BF16.
                dtype_after_modification = modified_tensor_output.dtype
                if dtype_after_modification not in (torch.float16, torch.float32):
                    final_tensor_for_numpy = modified_tensor_output.to(torch.float32)
                    logger.debug(f"Converted {new_name} from {dtype_after_modification} to {final_tensor_for_numpy.dtype} after modify_tensors (original input was {original_dtype})")
                else:
                    final_tensor_for_numpy = modified_tensor_output
                    # Log if modify_tensors itself changed the dtype from an unsupported one (like BF16)
                    # to a directly supported one (F16/F32) during its operation.
                    if final_tensor_for_numpy.dtype != original_dtype and original_dtype not in (torch.float16, torch.float32):
                         logger.debug(f"Dtype for {new_name} changed from {original_dtype} to {final_tensor_for_numpy.dtype} within modify_tensors.")

                # data = data_torch.squeeze().numpy() # Original comment
                data = final_tensor_for_numpy.numpy()

                # if data ends up empty, it means data_torch was a scalar tensor -> restore
                if len(data.shape) == 0:
                    data = final_tensor_for_numpy.numpy() # Use the same final_tensor_for_numpy
                ...
                # Log with original_dtype to show the true starting type of the tensor from the file
                logger.info(f"{f'%-{max_name_len}s' % f'{new_name},'} {original_dtype} --> {data_qtype.name}, shape = {shape_str}")
