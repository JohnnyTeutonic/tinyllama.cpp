...
#define CPPHTTPLIB_OPENSSL_SUPPORT  // Enable SSL if needed (requires OpenSSL
                                    // dev libraries)
...
int main(int argc, char** argv) {
  ...
  svr.Post(
      "/chat", [&session](const httplib::Request& req, httplib::Response& res) {
        ...
        // std::vector<std::pair<std::string, std::string>> history; // Original
        // placeholder
        ...
        try {
          ...
          if (req_json.contains("message")) {
            // This message field now contains the *entire* formatted prompt
            // from the client
          ...
          // if (req_json.contains("temperature")) temperature =
          // req_json["temperature"].get<float>(); if
          // (req_json.contains("max_new_tokens")) max_new_tokens =
          // req_json["max_new_tokens"].get<int>(); if
          // (req_json.contains("top_k")) top_k = req_json["top_k"].get<int>();
          // if (req_json.contains("top_p")) top_p =
          // req_json["top_p"].get<float>();
          ...
          Logger::info("Processing prompt received from client: " +
                       user_message.substr(0, 100) + "...");

          // Use the received message (which is the full formatted prompt)
          // directly
  ...
  unsigned int num_threads = std::max(
      1u, std::thread::hardware_concurrency() / 2);  // Use half the cores
