...
static int argmax(const std::vector<float>& v) {
  ...
  // Need <algorithm> for std::max_element and <iterator> for std::distance
...
static int sample_top_k_top_p_temperature(const std::vector<float>& logits,
  ...
  if (top_k <= 0)
    top_k = vocab_size;  // If top_k is non-positive, consider all tokens
  ...
  for (int i = 0; i < vocab_size; ++i) {
    scaled_logits[i] = (logits[i] - max_logit) /
                       std::max(temperature, 1e-6f);  // Avoid division by zero
  }
...
TinyLlamaSession::TinyLlamaSession(const std::string& model_path) {
  ...
  if (std::filesystem::is_directory(path_obj)) {
    ...
    try {
      ...
      ModelConfig loaded_config =
          parse_model_config(config_json);  // Use helper from model.h/cpp
  ...
  config_ = model_->get_config();  // Get config (parsed by model constructor
                                   // for GGUF, or loaded here for safetensors)
...
TinyLlamaSession::~TinyLlamaSession() {
  // unique_ptr handles deletion of model, tokenizer, cache (including CUDA
  // freeing via KVCache destructor)
  Logger::info("TinyLlamaSession: Destroyed.");
}

// --- GENERATE METHOD ---
std::string TinyLlamaSession::generate(const std::string& prompt, int steps,
                                       float temperature,
                                       const std::string& system_prompt) {
  Logger::info("Generate called. Prompt: \"" + prompt +
  ...
  for (int pos = 0; pos < total_steps; ++pos) {
...
#ifdef HAS_CUDA
    // --- CUDA Forward Path ---
    // Logger::info("API: Calling forward_device for pos " +
    // std::to_string(pos)); // Optional detailed log
    logits = model_->forward_device(input_token_id, pos, &kv_cache_, nullptr);
#else
    // --- CPU Forward Path ---
    // Logger::info("API: Calling forward (CPU) for pos " +
    // std::to_string(pos)); // Optional detailed log
    ...
    if (pos >= num_prompt_tokens - 1) {
      ...
      // Consider adding the sample_top_k_top_p_temperature function back if
      // needed
  ...
  std::vector<int> generated_only_ids(token_ids.begin() + num_prompt_tokens,
                                      token_ids.end());
  std::string result =
      tokenizer_->decode(generated_only_ids, true);  // Skip special tokens
