...
void matvec_f32_f32_cuda(cublasHandle_t handle,
  ...
  // Assuming A (mat_f32_dev) is row-major MxK, we transpose it to act
  // column-major KxM for cuBLAS
  status = cublasSgemm(
      handle,
...
void matvec_bf16_f32_cuda(cublasHandle_t handle, const uint16_t* mat_bf16_dev,
  ...
  // Use synchronous alloc/free here for simplicity as this path is less
  // critical
...
// Allocates temp device matrix AND vectors, copies data, calls device-pointer
// version, copies result back
void matvec_f32_f32_cuda(
    cublasHandle_t handle,
    const std::vector<float>& mat_f32_host,  // Changed to f32
    const std::vector<float>& vec_f32_host, std::vector<float>& out_f32_host,
    int rows, int cols) {
...
__global__ void softmax_normalize_kernel(const float* x, float* out, int n,
  ...
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < n) {
    // Read original value again to avoid needing large shared memory in kernel
    // 2/3
...
__global__ void rope_kernel(
    float* x, int num_heads, int head_dim,
  ...
  int pair_idx_in_head =
      idx % (head_dim / 2);  // Index within the head's pairs (0 to dim/2 - 1)

  // Calculate the offset into the *full* frequency buffer
  // Layout: [max_seq_len, head_dim / 2, 2] flattened -> [max_seq_len *
  // head_dim] Offset for current pos and current pair index:
  ...
  float x0 = x[base_x0];
  float x1 = x[base_x1];
  float cos_val = all_freqs_cis_base[freq_base_offset];  // cos value for (pos,
                                                         // pair_idx_in_head)
  float sin_val =
      all_freqs_cis_base[freq_base_offset +
                         1];  // sin value for (pos, pair_idx_in_head)
...
__global__ void attention_kernel(
    const float* Q_current,           // Shape [num_heads * head_dim]
    ...
    int num_q_heads)         // Num Query heads (gridDim.x)
{
  ...
  // 1. Compute attention scores for the current q_head against all keys in its
  // group
  float max_score = -INFINITY;
  const float* q_head_ptr = Q_current + q_head * head_dim;

  for (int k_pos = 0; k_pos < current_seq_len;
       ++k_pos) {  // Iterate up to current timestep
...
void attention_cuda(
    ...
    int cache_max_seq_len,  // Passed but not used directly in this wrapper
    int cache_num_kv_heads, cudaStream_t stream) {
...
__global__ void update_kv_cache_kernel(float* cache_base_ptr,
  ...
  // Assuming current_kv_vector points to the start of the data for the target
  // kv_head_idx
...
void update_kv_cache_cuda(
    float* cache_base_ptr,  // Base pointer for K or V for the layer
    const float* current_kv_head_vector,  // Pointer to current token's K or V
  ...
  update_kv_cache_kernel<<<gridDim, blockDim, 0, stream>>>(
      ...
      pos, kv_head_idx, max_seq_len, num_kv_heads, head_dim);
  gpuErrchk(cudaGetLastError());  // Check for errors after kernel launch
}
...
__global__ void rope_and_update_kv_cache_kernel(
    ...
    const float* kv_vector_head,  // Pointer to the *original* K or V data for
                                  // *this specific head*
    const float*
        all_freqs_cis_base,  // Base pointer to the global RoPE frequency buffer
  ...
  // Thread index corresponds to element pair index within the head dimension (0
  // to head_dim/2 - 1)
  ...
  // Indices into the source kv_vector_head (assumed tightly packed for this
  // head)
  int idx0 = pair_idx;
  ...
  // Offset into the *full* frequency buffer [max_seq_len * head_dim] for the
  // current pos and pair index
  size_t freq_base_offset = (size_t)pos * head_dim + (size_t)pair_idx * 2;
  float cos_val =
      all_freqs_cis_base[freq_base_offset];  // cos value for (pos, pair_idx)
  float sin_val = all_freqs_cis_base[freq_base_offset +
                                     1];  // sin value for (pos, pair_idx)
  ...
  size_t cache_offset_1 =
      cache_offset_0 + dim_half;  // Offset for the second element
...
__global__ void lookup_embedding_kernel(const void* __restrict__ table_dev,
  ...
  // Bounds check: Ensure the thread is within the hidden dimension
  if (idx >= hidden_size) {
    return;
  }

  // Bounds check: Ensure token_id is valid
  if (token_id < 0 || token_id >= vocab_size) {
    // Invalid token ID, write zero to the output
    output_dev[idx] = 0.0f;
    return;
  }
...
void lookup_embedding_cuda(const void* table_dev, float* output_dev,
  ...
  // --- Kernel Launch Configuration ---
  // Configure threads per block - 256 or 512 are common starting points
  int threads_per_block = 256;
  // Calculate blocks needed to cover the hidden_size dimension
  int blocks_per_grid =
      (hidden_size + threads_per_block - 1) / threads_per_block;

  // --- Launch Kernel ---
  // Logger::info("Launching lookup_embedding_kernel: Grid=" +
  // std::to_string(blocks_per_grid) + ", Block=" +
  // std::to_string(threads_per_block)); // DEBUG LOG
