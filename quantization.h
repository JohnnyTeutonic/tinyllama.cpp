...
static_assert(sizeof(block_q4_K) == 2 + 2 + 12 + 128,
              "Size mismatch for standard block_q4_K");  // Should be 144 bytes
...
static_assert(sizeof(block_q6_K) == 128 + 64 + 16 + 2,
              "Size mismatch for block_q6_K");  // Should be 210 bytes
...
static_assert(sizeof(block_q2_K) == 2 + 2 + 16 + 64,
              "Size mismatch for block_q2_K");  // Should be 84 bytes
...
struct block_q3_K {
  ...
  uint16_t
      dmin;  // Super-block min value (2 bytes) - Added dmin based on ggml.c
};
static_assert(sizeof(block_q3_K) == 32 + 64 + 12 + 2 + 2,
              "Size mismatch for block_q3_K");  // Should be 112 bytes
...
struct block_q8_K {
  ...
  int16_t bsums[GGML_QK_K /
                16];  // Block sums, used for dot product calculation (16 sums)
};

#define GGML_QK8_0 \
  32  // Standard block size for Q8_0 (make sure this matches ggml.c if used)
struct block_q8_0 {
...
size_t ggml_type_size(
    GGMLType type);  // Returns size in bytes of the *elements* (e.g., float=4)
size_t ggml_type_block_size(
    GGMLType type);  // Returns number of elements per block for block types
                     // (e.g., QK_K for _K types)
...
// Note: This is designed for testing and works with both block_q4_K and
// block_q4_K_test structs from the test file
...
void matvec_q6k_q8k_cpu(
    ...
    const std::vector<block_q8_K>&
        vec_q8k,                  // Q8_K input vector (quantized activation)
...
void dequantize_q8_0_block(const block_q8_0* qblock,
                           float* output);  // Dequantizes GGML_QK8_0 elements
