...
float fp16_to_fp32(
    uint16_t h,
    bool is_gguf_scale_field) {  // MODIFIED: Added is_gguf_scale_field
  ...
  if (is_gguf_scale_field && original_sign_bit_was_set) {
    ...
    // and proceed with the conversion as if the sign bit was 0, effectively
    // using magnitude. A debug log can be added here if needed to track when
    // this occurs. Logger::debug("[FP16_SCALE_PATCH] Original raw scale 0x" +
    // std::to_hex(h) + " had sign. Using magnitude.");
    h_to_convert = h & 0x7FFF;  // Mask off the sign bit
  }

  uint32_t sign = (h_to_convert >> 15) &
                  1;  // Will be 0 if sign was masked for scale fields
  ...
  if (exp_fp16 == 0) {     // Subnormal or zero
    if (mant_fp16 == 0) {  // Zero
      x = (sign << 31);  // sign will be 0 if it was a scale field with original
                         // sign set
    ...
    // For Inf/NaN, preserve original sign bit if it wasn't a scale field that
    // got masked. If it *was* a scale field and got masked, sign is now 0, so
    // it becomes +Inf/+NaN. If it was a scale field but
    // original_sign_bit_was_set was false, sign is also 0. This effectively
    // makes anomalous "negative Inf/NaN scales" into positive ones.
  ...
  // The old patch using std::abs() and std::cerr is now effectively replaced by
  // the is_gguf_scale_field logic for fields where it's true. For non-scale
  // fields (is_gguf_scale_field=false), if h had sign bit, f will be negative.
  // If we still want to ensure positive for scales even if a negative sneaks
  // through (shouldn't happen):
  if (is_gguf_scale_field && f < 0.0f && !(std::isnan(f) || std::isinf(f))) {
    // This case should ideally not be hit if h_to_convert had its sign bit
    // cleared. But as a safeguard for scales:
    // Logger::debug("[FP16_SCALE_PATCH_SAFEGUARD] Scale still negative: " +
    // std::to_string(f) + " from raw 0x" + std::to_hex(h) + ". Taking abs.");
    f = std::abs(f);
  }
...
uint16_t fp32_to_fp16(float f) {
  ...
  if (exp_fp32 == 0xff) {  // Inf or NaN
    u = (sign << 15) | 0x7c00 |
        (mant_fp32 != 0 ? 0x200 : 0);         // Preserve NaN payload slightly
    ...
    if (exp_fp16 >= 0x1f) {  // Overflow to Inf
      ...
      if ((mant_fp32 >> 13) & 1) {
        ...
        if ((mant_fp32 >> 23) == 1) {  // Check if mantissa overflowed
          ...
          exp_fp16++;
          if (exp_fp16 >=
              0x1f) {  // Check if exponent overflowed after rounding
            u = (sign << 15) | 0x7c00;
            return u;
          }
        }
...
namespace {  // Keep anonymous namespace for potentially internal-only helpers
             // in the future
...
static inline void get_scale_min_indices_q4_K(
  ...
  *scale_index &= 0x0F;  // Ensure only 4 bits are kept
  ...
  // Byte index cycles through 8..11 (j % 4 + 8). Nibble selected by j / 4 (0,
  // 1, 2, or 3).
...
static inline void get_scale_min_indices_q4_K(
    int j,                   // Sub-block index (0-15)
    const uint8_t* scales,   // Pointer to the 12-byte scales field in
block_q4_K uint8_t* scale_index,    // Output: 4-bit scale index (0-15) uint8_t*
min_index       // Output: 4-bit min index (0-15) ) { assert(j >= 0 && j < 16);
    int scale_byte_index = j / 2; // Index into scales bytes 0-5 // <<< WRONG
LOGIC int min_byte_index = j / 2 + 6; // Index into scales bytes 6-11 // <<<
WRONG LOGIC
...
// Dequantize Q4_K data - Corrected to match GGUF block layout with normalized
// sub-block scales
...
void dequantize_q6_k(const block_q6_K* qblock, float* output,
  ...
  // --- ADDED: Logging --- (Can be re-enabled/refined if needed after
  // correction) static std::atomic<int> log_count_q6 = 0; static
  // std::atomic<int> detailed_log_count_q6 = 0; bool should_do_detailed_log_q6
  // = log_this_block && (detailed_log_count_q6 < 2); if
  // (should_do_detailed_log_q6) { ... Logger ... }
  // --- END ADDED ---

  for (int sub_block_idx = 0; sub_block_idx < GGML_QK_K / 16;
       ++sub_block_idx) {  // Iterate 16 times for 16 sub-blocks of 16 weights
    const float s_sub_block = static_cast<float>(
        scales_ptr[sub_block_idx]);  // Current sub-block scale (int8_t to
                                     // float)
    const float effective_scale =
        d_super * s_sub_block;  // Effective scale for this sub-block

    // Pointers to the start of quant data for this sub-block
    const uint8_t* current_ql =
        ql_ptr +
        sub_block_idx * 8;  // Each sub-block of 16 weights uses 8 bytes from ql
    const uint8_t* current_qh =
        qh_ptr +
        sub_block_idx * 4;  // Each sub-block of 16 weights uses 4 bytes from qh
    float* current_output_ptr = output + sub_block_idx * 16;

    for (int N_in_subblock = 0; N_in_subblock < 16;
         ++N_in_subblock) {  // Iterate over 16 weights in the sub-block
      ...
      int quant_val =
          (q_high << 4) | q_low;  // Combine to form 6-bit value (0-63)

      current_output_ptr[N_in_subblock] =
          effective_scale * (static_cast<float>(quant_val) - 32.0f);

      // Optional detailed logging for the very first element of the very first
      // block if (log_this_block && sub_block_idx == 0 && N_in_subblock == 0 &&
      // detailed_log_count_q6 < 2) {
      //     std::stringstream detail_ss;
      //     detail_ss << "[DEQUANT_Q6K_DETAIL] First element calc (Overall
      //     Block #" << log_count_q6.load() << ", Sub #0, Elem #0):\n"
      //               << "  qblock->d_hex=0x" << std::hex << qblock->d <<
      //               std::dec << ", d_super=" << d_super << "\n"
      //               << "  sub_block_idx=" << sub_block_idx << ", scales_ptr["
      //               << sub_block_idx << "]=" <<
      //               static_cast<int>(scales_ptr[sub_block_idx]) << "
      //               (s_sub_block=" << s_sub_block << ")\n"
      //               << "  effective_scale=" << effective_scale << "\n"
      //               << "  N_in_subblock=" << N_in_subblock << " ->
      //               ql_byte_offset=" << (N_in_subblock/2) << "
      //               qh_byte_offset=" << (N_in_subblock/4) << "\n"
      //               << "  q_low_byte=0x" << std::hex << (int)q_low_byte << ",
      //               q_high_byte=0x" << (int)q_high_byte << std::dec << "\n"
      //               << "  q_low_bits=" << q_low << ", q_high_bits=" << q_high
      //               << " -> quant_val=" << quant_val << " (val-32: " <<
      //               (static_cast<float>(quant_val) - 32.0f) << ")\n"
      //               << "  Output = " << effective_scale << " * (" <<
      //               (static_cast<float>(quant_val) - 32.0f) << ") = " <<
      //               current_output_ptr[N_in_subblock];
...
// It doesn't implement sophisticated quantization strategies like those in
// ggml.c
void quantize_q4_k_m(const float* input, void* output_qblock_void,
  ...
  // Step 1: Find min/max for the entire block to set super-block scale (d) and
  // min (dmin)
  ...
  // Super-scale should ideally map the *absolute* range to the scale table
  // factors (1.0 to 15.5) Super-min should ideally map the *actual min* to the
  // min table factors (0.0 to -1.8125)
  const float d_super_scale_candidate =
      (block_max_val - block_min_val) / 15.0f;  // A potential scale measure
  const float d_super =
  ...
  for (int j = 0; j < GGML_QK_K / 16; ++j) {  // j = 0..15
    ...
    if (sub_max_val >
        sub_min_val +
            1e-10f) {  // Avoid division by zero if sub-block is constant
    ...
    uint8_t packed_qs[8];  // Temporary storage for packed nibbles for this
                           // sub-block
    ...
    // g) Copy the packed nibbles for this sub-block into the correct location
    // in output_qblock->qs Original packing logic had high/low swapped,
    // corrected here: Low nibble is index i, High nibble is index i+8
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      qs_target[i] =
          low_nibble_val |
          (high_nibble_val << 4);  // Pack low first, then high shifted
    }
  }
}
...
void dequantize_q2_k(const void* qblock_void,
  ...
  const uint8_t* scales_ptr =
      qblock->scales;  // 16 bytes, each contains two 4-bit scales
  const uint8_t* qs_ptr =
      qblock->qs;  // 64 bytes, each contains four 2-bit weights
  int weight_index = 0;
  ...
  for (int i = 0; i < 8; ++i) {  // Process 8 bytes = 16 scales
    ...
    // According to GGML, Q2_K 4-bit scales use d_float directly (no lookup
    // table or dmin here) Use the clamped value for safety
  ...
  // Step 2: Dequantize the 2-bit weights using the dequantized scales and
  // global dmin
  weight_index = 0;
...
void dequantize_q3_k(const void* qblock_void, float* output,
  ...
  for (int j = 0; j < GGML_QK_K / 16; ++j) {  // j = 0..15
    ...
    if (j < 8) {
      ...
      scale_idx =
          scales_ptr[j + 4] & 0x3F;  // Lower 6 bits from the second half
    }
    ...
    // Q3_K does NOT use a separate minimum factor from a table. It calculates
    // min implicitly. The minimum value is implicitly represented by the
    // quantization grid. We only need the scale factor for the dequantization
    // formula.
    const float final_sub_block_scale = d_float * sub_block_scale_factor;
    const float final_sub_block_min =
        dmin_float;  // The global dmin acts as the base minimum offset.
...
size_t ggml_type_size(GGMLType type) {
  switch (type) {
    ...
    case GGMLType::GGML_TYPE_Q8_0:
      return 34;  // Example size (1*scale_fp32 + 32*qs_int8)
    case GGMLType::GGML_TYPE_Q8_1:
      return 40;  // Example size (1*scale_fp32 + 32*qs_int8)
    case GGMLType::GGML_TYPE_Q5_K:
      return 116;  // Example size (adjust based on actual struct)
    case GGMLType::GGML_TYPE_Q8_K:
      return 290;  // Example size (adjust based on actual struct)
    case GGMLType::GGML_TYPE_I16:
      return sizeof(int16_t);
    case GGMLType::GGML_TYPE_I32:
      return sizeof(int32_t);
    case GGMLType::GGML_TYPE_BF16:
      return sizeof(uint16_t);       // <<< ADDED BF16 (size 2) >>>
...
std::vector<block_q8_K> quantize_fp32_to_q8_K(
  ...
  for (size_t i = 0; i < num_blocks; ++i) {
    ...
    const float d_fp32 =
        amax / 127.0f;  // This is the scale before FP16 conversion
    ...
    if (log_count_q8k_quant_scales <
        10) {  // Log for the first few blocks quantized
    ...
    for (int j = 0; j < GGML_QK_K; ++j) {
      ...
      y[i].qs[j] = q_val;
      block_sum[j / 16] +=
          q_val;  // Add to the sum for the corresponding 16-element block
    }
...
float vec_dot_q6_k_q8_k_cpu(
  ...
  for (size_t i = 0; i < nb; ++i) {
    ...
    for (int l = 0; l < 8; ++l) {
      float term =
          d * (aux32[l] -
               32 * sumi_mins / 8);  // Distribute compensation across aux32
      sums[l] += term;
...
void dequantize_q8_k(const std::vector<block_q8_K>& q_data,
  ...
  for (size_t i = 0; i < num_blocks; ++i) {
    ...
    const float d = fp16_to_fp32(
        qblock->d, true);  // MODIFIED: Pass true for is_gguf_scale_field
...
void dequantize_q8_0_block(const block_q8_0* qblock, float* output) {
  const float d_fp32 =
      fp16_to_fp32(qblock->d, true);      // GGUF scale field is true
