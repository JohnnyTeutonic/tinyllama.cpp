...
// Capitalizes the first logical letter of a string, handling SentencePiece
// space prefix. Note: Assumes the first letter is single-byte ASCII/Latin for
// simplicity.
std::string capitalize_first_letter(const std::string& s) {
  ...
  if (result.rfind(sp_space, 0) == 0) {
    if (result.length() > sp_space.length()) {
      first_letter_pos =
          sp_space.length();  // Capitalize after the space prefix
    } else {
...
Tokenizer::Tokenizer(const GGUFData& gguf_data) : initialized_from_gguf_(true) {
  ...
  if (gguf_data.tokenizer_tokens.size() != gguf_data.tokenizer_scores.size() &&
      !gguf_data.tokenizer_scores.empty()) {  // CORRECTED (x3)
    Logger::warning(
        "GGUF token and score array sizes mismatch: tokens=" +
        std::to_string(gguf_data.tokenizer_tokens.size()) + ", scores=" +
        std::to_string(gguf_data.tokenizer_scores.size()));  // CORRECTED (x2)
  }
  ...
  if (pre_tok_type_ == "unknown") {
    ...
    if (arch == "llama") {
      pre_tok_type_ =
          "llama";  // Assume llama pre-tok for llama arch if not specified
  ...
  added_tokens_.clear();  // Clear any previous added tokens (e.g., from default
                          // constructor)
  ...
  if (!token_types_.empty() && token_types_.size() == id_to_token_.size()) {
    ...
    for (size_t i = 0; i < token_types_.size(); ++i) {
      ...
      // Add if it's marked as CONTROL/USER_DEFINED OR if it's one of the known
      // special IDs (BOS/EOS etc.) This ensures things like [INST] are added if
      // marked as CONTROL, and <s> is added even if marked NORMAL
    ...
    // Manually add BOS/EOS/UNK/PAD to added_tokens_ if they were found by ID
    // earlier
...
std::vector<std::string> Tokenizer::regex_tokenize(
    const std::string& text) const {  // Ensure 'const' is present
  std::vector<std::string> tokens;
  // Regex pattern similar to llama.cpp: optional space followed by non-space
  // characters This helps handle the leading space convention in many BPE
  // tokenizers. Need double escapes for backslashes in C++ string literals.
  try {
...
std::vector<std::string> Tokenizer::bpe_tokenize(
  ...
  for (auto& word : words) {
    ...
    if (using_space_prefix && word.size() > 0 && word[0] == ' ') {
      ...
      // Update trace flag if the original word was the target, AFTER
      // replacement
      trace_this_word =
          (word == "\xE2\x96\x81Studying");  // NEW Check for Studying
...
std::vector<std::string> Tokenizer::sentencepiece_tokenize(
    const std::string& text) const {
  // Fallback to space tokenization until SentencePiece implementation is
  // complete
  Logger::info(
    ...
    if (added_it != added_tokens_.end()) {
      ...
      if (base_it != token_to_id_.end()) {
        ...
        if (capitalized_token !=
            token) {  // Only try if capitalization actually changed something
...
std::vector<std::string> Tokenizer::ids_to_tokens(
  ...
  for (int id : ids) {
    ...
    if (added_it != id_to_added_token_.end()) {
      ...
      // Logger::debug("ID " + std::to_string(id) + " -> Added Token: '" +
      // added_it->second + "'");
      ...
      if (!id_to_token_[id].empty()) {  // Check if token string is valid
        ...
        // Logger::debug("ID " + std::to_string(id) + " -> Base Token: '" +
        // id_to_token_[id] + "'");
      } else {
        // ID is within base vocab range but has an empty string (shouldn't
        // happen often)
...
std::vector<int> Tokenizer::encode(const std::string& text, bool add_bos,
  ...
  if (!initialized_from_gguf_) {
    ...
    std::vector<std::string> bpe_pieces =
        bpe_tokenize(text);  // Call the restored bpe_tokenize
    ...
    std::vector<std::pair<std::string, bool>>
        segments;  // Pair: <text_segment, is_special>
    ...
    if (method_to_use == PreTokenizeMethod::DEFAULT) {
      ...
      // Build regex pattern for splitting by *all* special tokens: those with
      // IDs (added_tokens_) and those from chat_template_special_tokens
        ...
        if (is_special) {
          ...
          if (initialized_from_gguf_) {
            ...
            bpe_pieces = bpe_tokenize(
                segment_to_process);  // For 'default', use whitespace BPE
          }
...
std::string Tokenizer::apply_chat_template(const std::string& user_prompt,
    ...
    const  // <<< Keep config for potential future use?
{
  ...
  auto find_added_token_str = [&](const std::string& content,
    ...
    // Log only if fallback is used AND the token map isn't empty (i.e.,
    // expected to be found)
    if (!added_tokens_.empty()) {
      Logger::warning("Could not find added token '" + content +
                      "' in added_tokens_ map. Using fallback: '" + fallback +
                      "'");
    }  // else: map is empty (e.g., NoAdded case), fallback is expected, don't
       // warn.
        ...
        for (const auto& token_obj : added_tokens) {
          if (token_obj.contains("content") && token_obj.contains("id")) {
            ...
            Logger::info("Processed added token: " + token + " with ID " +
                         std::to_string(id));  // Changed log msg
          }
        }
      }
...
void Tokenizer::load_bpe_merges_from_json(const std::string& model_path) {
  try {
    ...
    if (model_json.contains("model") && model_json["model"].contains("type") &&
      ...
      // The last part of the vocabulary has entries with spaces that appear to
      // be the merges
      else {
        ...
        int merge_index = 0;
        // Look for patterns in the vocab that have spaces - these are likely
        // merge pairs
        if (model_json["model"].contains("vocab") &&
          ...
          for (auto it = vocab.begin(); it != vocab.end(); ++it) {
            ...
            if (space_pos != std::string::npos) {
              ...
              // Some models use priority based on vocab ID, others use
...
struct BPEMerge {
  ...
  int index;    // Index of the first token in the pair within the current
                // sequence
...
std::vector<std::string> Tokenizer::bpe_tokenize_from_scores(
  ...
  for (const std::string& unit_raw : filtered_units) {  // Use filtered_units
    ...
    if (using_space_prefix && unit.length() > 0 && unit[0] == ' ') {
      ...
      // Logger::debug("[BPE_TOKENIZE] Replaced leading space with SP space: '"
      // + unit_raw + "' -> '" + unit + "'");
