...
SafeTensorsLoader::SafeTensorsLoader(const std::string& path) {
    ...
    if (file.gcount() != sizeof(header_len)) throw std::runtime_error("Failed to read safetensors header length");

    // Read header JSON
    std::vector<char> header_buf(header_len);
    file.read(header_buf.data(), header_len);
    if (file.gcount() != static_cast<std::streamsize>(header_len)) throw std::runtime_error("Failed to read safetensors header");
...
std::vector<uint8_t> SafeTensorsLoader::get_tensor_bytes(const std::string& name) const {
    ...
    if (!file) throw std::runtime_error("Failed to open safetensors file: " + file_path_);
    ...
    if (file.gcount() != static_cast<std::streamsize>(info.nbytes)) throw std::runtime_error("Failed to read tensor data: " + name);
    return data;
}
