...
void Logger::log_vector_stats(const std::string& name, const std::vector<float>& v, int n_show) {
    ...
    for (int i = 0; i < n_show && i < v.size(); ++i) first_vals += std::to_string(v[i]) + " ";
    info(name + ": min=" + std::to_string(minv) + ", max=" + std::to_string(maxv) + ", mean=" + std::to_string(mean) + ", all_finite=" + (all_finite ? "yes" : "no") + ", first_vals=" + first_vals);
} 